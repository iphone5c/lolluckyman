...
public class LolTopUpWithdrawalControllerJSON extends BaseController {
    ...
    public Object applyTopUp(HttpServletRequest request,HttpServletResponse response,Double applyMoney ,String tradeType,String tradeNumber){
        if (applyMoney==null||applyMoney<=0)
            return validationResult(1001,"充值金额必须大于零");
        if (LolUtils.isEmptyOrNull(tradeType))
            return validationResult(1001,"交易类不能为空，请选择（支付宝、微信）");
        if (LolUtils.isEmptyOrNull(tradeNumber))
            return validationResult(1001,"交易账号不能为空");
        ...
        if (topUpWithdrawal!=null){
            return result("充值申请已完成，等待处理。");
        }else{
            return validationResult(1001,"充值申请失败");
        }
    }
}
